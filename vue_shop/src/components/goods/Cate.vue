    ...
    <el-card>
      ...
      >
        ...
        <template slot="opt" slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="showCateEditDialog(scope.row.cat_id)"
            >编辑</el-button
          >
...
export default {
  ...
  methods: {
    ...
    // 选择项发生变化触发这个函数
    parentCateChanged() {
      // 如果 selectedKrys 数组中的 length 大于0，证明选中了父级分类
      // 反之，就说明没有选中任何父级分类
    ...
    editCateInfo() {
      this.$refs.editCateFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.put(
          ...
          { cat_id: this.editCateForm.cat_id , cat_name: this.editCateForm.cat_name }
        );
        if (res.meta.status !== 200) {
